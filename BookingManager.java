...
public class BookingManager{
    ...
    public void createBooking(String bookingId, Resident resident, ChargingStation station, LocalDate date, LocalTime start, LocalTime end)
    {
        try {
            ...
            Booking newBooking = new Booking(bookingId, resident, station, date, start, end);
    ...
    public boolean hasActiveBooking(String stationId) {
        return getActiveBooking(stationId) != null;
    }

    public Booking getActiveBooking(String stationId) {
        for (Booking b : bookingList) {
            if (b.getStatus().equals("Active") && b.getStation().getStationId().equalsIgnoreCase(stationId)) {
                return b;
            }
        }
        return null;
    ...
    public ArrayList<Booking> getActiveBookingsForResident(Resident resident) {
        ArrayList<Booking> result = new ArrayList<>();
        for (Booking b : bookingList) {
            if (b.getStatus().equals("Active") && b.getResident().getResidentId().equalsIgnoreCase(resident.getResidentId())) {
                result.add(b);
            }
        }
        return result;
    ...
    public void viewMyBookings(Resident loggedInResident) {
        ...
        for (Booking b : bookingList) {
            ...
            if (ownerId.equalsIgnoreCase(myId) && !b.getStatus().equals("Cancelled")) {
                System.out.println(b.toString());
                System.out.println("Duration: " + b.getDuration() + " minutes\n");
            ...
            while (scan.hasNextLine()) {
                ...
                if (data.length == 7) {
                    try {
                        ...
                        Booking b = new Booking(bookingId, loadedResident, loadedStation, date, startTime, endTime);
                        b.setStatus(status); 

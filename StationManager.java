...
public class StationManager {
    ...
    public void addStation(String stationId, String type, String location, double powerOutputKw)
            throws DuplicateStationException, InvalidStationTypeException {
        if (findStation(stationId) != null) {
            throw new DuplicateStationException("Station ID '" + stationId + "' already exists.");
        }

        if (type == null || !(type.equalsIgnoreCase("Fast") || type.equalsIgnoreCase("Normal"))) {
            throw new InvalidStationTypeException("Station type must be either 'Fast' or 'Normal'.");
        }
    ...
    public void removeStation(String stationId) throws StationNotFoundException, InvalidStatusUpdateException {
        ChargingStation station = findStation(stationId);
        if (station == null) {
            throw new StationNotFoundException("Station '" + stationId + "' not found.");
        }
        if (bookingManager != null && bookingManager.hasActiveBooking(stationId)) {
            throw new InvalidStatusUpdateException(
                "Station '" + stationId + "' has a current booking and cannot be removed.");
        }
    ...
    public void resolveMaintenance(String stationId) throws StationNotFoundException, InvalidStatusUpdateException {
        ChargingStation station = findStation(stationId);
        if (station == null) {
            throw new StationNotFoundException("Station '" + stationId + "' not found.");
        }
        if (!station.isUnderMaintenance()) {
            throw new InvalidStatusUpdateException("Station '" + stationId + "' is not currently under maintenance.");
        }
        station.setUnderMaintenance(false);
